...
# We create a temporary list to write all of the vars that are within the shared set of positions
        ...
        for isolate in strain_dict[culture]:
            ...
            var_file = open(isolate + ".no-indelSNP.var", 'r')
            isolate_var_set = set()
...
# that python would recognize isolate_1, for instance, as a variable.  I'm sure there is a way to do this.
    # for isolate_1:
        ...
        var_file = open(isolate_1 + ".no-indelSNP.var", 'r')
        unique_var_file = open(isolate_1 + "_uniqueindelSNP.var", "w")
        branch_var_file = open(isolate_1 + "_branchindelSNP.var", "w")
        shared_var_file = open(isolate_1 + "_trunkindelSNP.var", "w")
        oneVtwo_file = open(isolate_1 + "_1v2_indelSNP.var", "w")
        oneVthree_file = open(isolate_1 + "_1v3_indelSNP.var", "w")
    ...
    # for isolate_2:
        ...
        var_file = open(isolate_2 + ".no-indelSNP.var", 'r')
        unique_var_file = open(isolate_2 + "_uniqueindelSNP.var", "w")
        branch_var_file = open(isolate_2 + "_branchindelSNP.var", "w")
        shared_var_file = open(isolate_2 + "_trunkindelSNP.var", "w")
        oneVtwo_file = open(isolate_2 + "_1v2_indelSNP.var", "w")
        twoVthree_file = open(isolate_2 + "_2v3_indelSNP.var", "w")
    ...
    # for isolate_3:
        ...
        var_file = open(isolate_3 + ".no-indelSNP.var", 'r')
        unique_var_file = open(isolate_3 + "_uniqueindelSNP.var", "w")
        branch_var_file = open(isolate_3 + "_branchindelSNP.var", "w")
        shared_var_file = open(isolate_3 + "_trunkindelSNP.var", "w")
        oneVthree_file = open(isolate_3 + "_1v3_indelSNP.var", "w")
        twoVthree_file = open(isolate_3 + "_2v3_indelSNP.var", "w")
        ...
        column_titles = ['1_allindelSNPs','1_branchindelSNPs','1_uniqueindelSNPs','2_allindelSNPs','2_branchindelSNPs','2_uniqueindelSNPs','3_allindelSNPs','3_branchindelSNPs','3_uniqueindelSNPs','1 v 2 only','1 v 3 only','2 v 3 only','trunkindelSNPs']

# This creates a dataframe to report all mutations from each strain found in the shared genome at different branch points.
        df = pd.DataFrame({'1_allindelSNPs' : pd.Series(isolate_1_all_vars,index=isolate_1_all_vars),
                            '1_branchindelSNPs' : pd.Series(isolate_1_branch_vars,index=isolate_1_branch_vars),
                            '1_uniqueindelSNPs' : pd.Series(isolate_1_unique_vars,index=isolate_1_unique_vars),
                            '2_allindelSNPs' : pd.Series(isolate_2_all_vars,index=isolate_2_all_vars),
                            '2_branchindelSNPs' : pd.Series(isolate_2_branch_vars,index=isolate_2_branch_vars),
                            '2_uniqueindelSNPs' : pd.Series(isolate_2_unique_vars,index=isolate_2_unique_vars),
                            '3_allindelSNPs' : pd.Series(isolate_3_all_vars,index=isolate_3_all_vars),
                            '3_branchindelSNPs' : pd.Series(isolate_3_branch_vars,index=isolate_3_branch_vars),
                            '3_uniqueindelSNPs' : pd.Series(isolate_3_unique_vars,index=isolate_3_unique_vars),
                            ...
                            'trunkindelSNPs' : pd.Series(shared_vars,index=shared_vars)}).reindex(total_var_list)
...
# This step is necessary because the columns aren't indexed in df in the order they are entered.
    ...
    df_master.to_csv("testindel.csv")
